    ...
    <script>
        ...
        M.login = function() {
            ...
            if (code !== null) {
                $.ajax({
                    type: "POST",
                    url: "{:$router->buildUrl('/ebcms/ucenter-web/auth/login')}",
                    data: {
                        code: code,
                    },
                    dataType: "JSON",
        ...
        $(function() {
            ...
            $("#form").bind('submit', function() {
                $.ajax({
                    type: "POST",
                    url: "{:$router->buildUrl('/ebcms/ucenter-web/auth/send-code')}",
                    data: $("#form").serialize(),
                    dataType: "JSON",
                    success: function(response) {
                        $("#captcha").trigger('click');
...
<body>
    <div class="my-3" style="max-width: 400px; margin:0 auto;">
        <div class="display-5 my-4 text-center">用户登录</div>
                ...
                <div class="mb-4">
                    ...
                    <div class="input-group mb-2 me-sm-2">
                        ...
                        <div class="input-group-append">
                            <img id="captcha" style="vertical-align: middle;cursor: pointer;height: 48px;" class="rounded-right" onclick="this.src = '<?php echo $router->buildUrl('/ebcms/ucenter-web/auth/captcha'); ?>?time=' + (new Date()).getTime();">
                        </div>
                    </div>
                </div>
