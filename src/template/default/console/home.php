{include console/header@ebcms/ucenter-web}
<div class="display-4 my-4">个人中心</div>
<hr>
<div class="my-3 h3">欢迎你</div>
<div class="d-flex">
    <img style="width: 90px;height: 90px;" class="me-3" src="{$my['avatar']?:'data:image/svg+xml;base64,'}" alt="...">
    <div class="media-body">
        <h5 class="mt-0 mb-1">{$my.nickname}</h5>
        <div>
            <code><b>电话：</b>{:substr($my['phone'], 0, 3)}****{:substr($my['phone'], 7)} <b>金币：</b>{$my.coin}</code>
        </div>
        <div class="mt-1">{$my['introduction']?:'暂无介绍'}</div>
    </div>
</div>
<div class="mt-3">
    <a href="{:$router->buildUrl('/ebcms/ucenter-web/console/edit-info')}" class="btn btn-primary btn-sm">编辑个人信息</a>
</div>
{include console/footer@ebcms/ucenter-web}